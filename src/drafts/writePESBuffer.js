export function writePESBuffer(polylines) {
  const stitches = convertGeometryToStitches(polylines);
  const pesBuffer = createPESFile(stitches);
  return pesBuffer;
}

function convertGeometryToStitches(polylines) {
  const stitches = [];
  ...
  for (const polyline of polylines) {
    ...
    // First point of each polyline needs trim and special encoding
    const firstPoint = polyline.points[0];
    const startX = Math.round(firstPoint[0] * SCALE);
    const startY = Math.round(firstPoint[1] * SCALE);
    ...
    // Process remaining points in the polyline
    for (let i = 1; i < polyline.points.length; i++) {
      const point = polyline.points[i];
      const targetX = Math.round(point[0] * SCALE);
      const targetY = Math.round(point[1] * SCALE);
